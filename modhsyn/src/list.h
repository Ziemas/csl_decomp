...
static inline void
list_insert(struct list_head *list, struct list_head *node)
{
...
static inline void
list_join(struct list_head *prev, struct list_head *next)
{
	prev->next = next;
	next->prev = prev;
}
...
static inline void
list_remove(struct list_head *node)
{
...
static inline int
list_node_is_last(struct list_head *list)
{
	return list->next == list->prev;
}

static inline int
list_same_node(struct list_head *node1, struct list_head *node2)
{
	return node1 == node2;
}
...
#define list_for_each_it(var, list)                          \
	for ((var) = (list)->next; !list_same_node((var), list); \
		 (var) = (var)->next)
...
#define list_for_each_safe_it(var, n, list)       \
	for ((var) = (list)->next, (n) = (var)->next; \
		 !list_same_node((var), list); (var) = (n), (n) = (var)->next)

#define list_for_each_safe(var, head, member)                           \
	typeof((var)) var##__safe;                                          \
	for ((var) = list_first_entry(head, typeof(*(var)), member),        \
		(var##__safe) = list_next_entry(var, member);                   \
		 !list_entry_is_head(var, head, member); (var) = (var##__safe), \
		(var##__safe) = list_next_entry(var##__safe, member))

static inline int
list_length(struct list_head *list)
{
	struct list_head *i;
	int len = 0;
