...
#define TM_NO_GATE (0)
    ...
    ((sceCslMidiStream*)stream_buf)->buffsize = STREAMBUF_SIZE + sizeof(sceCslMidiStream);
...
s32* read_file(const char* filename)
{
    io_stat_t stat;
...
int _start()
{
    ...
    header = (struct entry_header*)((u8*)header + header->entry_byte_count + header->header_byte_count);
    printf("name %s\n", header->name);
    ...
    sceMidi_MidiSetVolume(&midi_ctx, 0, sceMidi_MidiSetVolume_MasterVol, sceMidi_Volume0db);
